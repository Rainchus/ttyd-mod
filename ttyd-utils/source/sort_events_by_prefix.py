...
for key_folder, subfolders in prefix_subdirs.items():
    for subfolder in subfolders:
        search_path = os.path.join(path_to_files, key_folder, subfolder)
        if os.path.exists(search_path):
            ...
            combined_file_path = os.path.join(search_path, f"__combined_{subfolder}.txt")

            # Skip processing the combined file if it already exists in the list of files
            if combined_file_path in txt_files:
                txt_files.remove(combined_file_path)

            with codecs.open(combined_file_path, "w", encoding="utf-8") as combined_file:
                for txt_file in txt_files:
                    with codecs.open(txt_file, "r", encoding="utf-8", errors="ignore") as input_file:
                        combined_file.write(input_file.read())
                        combined_file.write("\n")  # Add a newline between files

            print(f"Combined text files in {search_path} into __combined_{subfolder}.txt")